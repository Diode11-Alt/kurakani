...
export async function decryptMessage(
  ...
  let ciphertext = ciphertextBase64;
  try {
    // Try converting from Base64 (new messages)
    if (ciphertextBase64 && !ciphertextBase64.includes(' ') && /^[A-Za-z0-9+/=]+$/.test(ciphertextBase64)) {
      ciphertext = atob(ciphertextBase64);
    }
  } catch (e) {
    // Fall back to assuming it's already a binary string (older messages)
    console.warn("atob failed, falling back to raw string", e);
  }
