...
export function VideoCall({
  ...
  const callStateRef = useRef<typeof callState>(callState);
  ...
  const getIceServers = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // For remote users (different network, city, country) this MUST be a real public address.
        ...
        // Also add TCP variant for strict firewalls (port 443 is rarely blocked)
        servers.push({
    ...
    // Fallback TURN servers for remote users who can't reach your self-hosted Coturn.
    // openrelay.metered.ca uses shared public credentials and is heavily rate-limited —
    // it WILL fail for real users. Use NEXT_PUBLIC_METERED_API_KEY for a real free-tier key
    // from https://dashboard.metered.ca or host your own Coturn with a proper public IP/domain.
    ...
    if (meteredKey) {
      ...
      console.warn('[ICE] No NEXT_PUBLIC_METERED_API_KEY set. Remote calls (different network/city) will likely fail. Set up a real TURN server.');
      servers.push(
        {
          ...
          case 'answer':
            if (peerConnectionRef.current && (callStateRef.current === 'ringing-out' || callStateRef.current === 'connecting')) {
              ...
              try {
                ...
                // Set connected AFTER remote description successfully applied
                setCallState('connected');
              } catch (err) {
  ...
  const acceptIncomingCall = async () => {
    ...
    try {
      ...
      // Do NOT setCallState('connected') here — ICE negotiation hasn't finished yet.
      // For remote users on different networks, ICE can take several seconds.
      // onconnectionstatechange → 'connected' will fire when media path is ready.
      setCallState('connecting');
      flushLocalCandidates();
    } catch (err) {
