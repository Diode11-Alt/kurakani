  ...
  const loadProfileData = async () => {
    try {
      ...
      if (currentUserId && currentUserId !== profileId) {
        ...
        const { data: conn } = await (await import('../../../../lib/supabase')).supabase
      ...
      <div className="px-4 pt-4 pb-6 border-b border-[var(--color-outline-variant)]/40">
        ...
        <div className="mb-4 sm:px-2">
          ...
          <div className="text-[13px] sm:text-sm text-[var(--color-on-surface-variant)] mb-1">
            <span className="text-[var(--color-on-surface-variant)]">@{profile.username}</span> 
            {profile.isPublic === false && <span className="ml-2 text-xs bg-[var(--color-surface-container-highest)] px-2 py-0.5 rounded-full">Private Account</span>}
          </div>
          ...
          {isPrivateAndUnconnected ? (
            <div className="py-4 text-center border border-[var(--color-outline-variant)] rounded-xl bg-[var(--color-surface-container-lowest)] mt-4">
              ...
              <p className="text-sm font-bold text-[var(--color-on-surface)]">This account is private</p>
              <p className="text-xs text-[var(--color-on-surface-variant)] mt-1">Connect with them to see their full profile and send messages.</p>
            </div>
          ) : (
            <>
              {profile.bio && (
        ...
        <div className="flex gap-2 w-full sm:px-2">
          {!isOwnProfile ? (
            <>
              ...
              {profile.requireConnectionRequests && connectionStatus !== 'accepted' ? (
                <button
      ...
      <div className="pb-10 w-full max-w-full">
        {isPrivateAndUnconnected ? (
          <div className="flex flex-col items-center justify-center py-20 text-[var(--color-on-surface-variant)]">
            ...
            <h3 className="text-xl font-bold text-[var(--color-on-surface)] mb-2">Private Account</h3>
          </div>
        ) : (
          <>
            {activeTab === 'posts' && (
